...
-- Stacks,Display Name, ConErr, LigErr, PwrErr, RadioActive 1Y 0N, System Name 
                      VAL = {"Default                     ",0,0,0,0,"Default"}
-- Gases ------------------------------------------------------------------------------------------------
                 Nitrogen = {"Nitrogen Tank 1             ",0,0,0,0,"Nitrogen"}
--#######################################################################################################

-- add below what each container is in the format below:

-- Examples -- ###################################################################
-- Tanks Gases  = GasStatus(Line,Contents,Alarm)
-- ###############################################################################

function ITEMLIST()

LiqStatus(8,Nitrogen,false)
...
text.size = PAN[6]
text.text = "Gas Manager"
dev = 0
local ProgName = ("Ficsit Gas Manager 3030     ")
local By = ("Skyamoeba")
local Ver = ("1.0.0 ")
...
ModVersion = MODVerD()
ModVerPrint= MODVerP()
VersionFluid= VerCheckGasD()
VerPrint = VerCheckGasP()
...
function LiqData()
prefix = {"GAS", "LIG", "PWR"}
local setupgas = {prefixcon= prefix[1], condata=Contents[6]}
...
LiqLvl = round(RawLvl)
FlowIn = Gas.flowFill
FlowOt = Gas.flowDrain
MaxFlo = Gas.flowLimit
end
